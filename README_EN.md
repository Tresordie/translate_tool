...
- **Quick Actions** — Swap languages, paste from clipboard, clear, copy the source text or the result; pasting is one click with no repeated clipboard-permission prompts
...
### v0.25.11 (2026-09-11)
- **New: one-click "WeChat format" in four modules (Work Report / Email Summary / AI Parse / AI Prompts)** — a "微信格式" button in the result button row converts the AI output into plain text shown in a new region below the result, with its own Copy button, ready to paste into WeChat. WeChat does not render Markdown, so syntax markers are removed while emoji and structure are kept: headings drop `#` but keep their emoji, unordered lists become `•`, task items become `☑/☐`, bold numbering `**1. Title** detail` becomes "1. Title: detail", quotes become 「」, tables become "value | value", links become "text (url)". Emoji are processed at string level (surrogate pairs are never split) and only characters WeChat renders as boxes/ghosts are removed (variation selectors `U+FE0E/FE0F`, zero-width joiner `U+200D`, enclosing keycap `U+20E3`); code-block bodies are kept verbatim without indentation so generated prompts stay usable. The converter lives in the shared `markdown.js` (`window.markdownToWechat`) used by both the web app and the extension copies.
- **New: copy button for the translation source box** — the source toolbar in the web app and the extension (popup / full-page / side panel) gains a copy button that copies the source text in one click (empty input shows a toast and leaves the clipboard untouched).
- **Fix: Work Report "output language English but Chinese produced"** — the cause was config sync: `applySyncedConfig` reset `config.outputLang` to `zh` on every extension broadcast and never wrote it back to the select, so the UI showed English while prompt building read the reset `zh` and took the Chinese branch. Generation now reads the select's current value, and config sync only updates Base URL / API Key / model while preserving the language preference and writing it back to the select.
- **Fix: paste button re-prompting for clipboard permission every time** — `navigator.clipboard.readText()` is permission-gated: a `file://` page is an opaque origin whose grant is never remembered, so every call prompts; extension pages prompted too because `manifest.json` did not declare `clipboardRead`. The extension now declares `clipboardRead`, and the web app reads the clipboard through a new content-script `read-clipboard` bridge that uses `execCommand('paste')` on the extension's behalf (one-click paste, no prompt); without the extension, or when the permission state is `prompt`, it focuses the editor and asks for `Ctrl+V` instead — **no authorization dialog is triggered in any case**. Extension pages (full-page / side panel) now prefer `execCommand('paste')` with a `readText()` fallback. ⚠️ **Reload the extension in `chrome://extensions`** (new permission).
- **New: popup "Open local web version" entry** — a new popup header button opens the local `index.html` in a new tab; the path is configurable in Settings under "Local page path" (defaults to the repo's local `index.html`), with a clear hint when "Allow access to file URLs" is disabled.
- **Fix: first-open config race** — pages now actively request the config once on init (a `request-config` handshake), avoiding the race where the content script's async `localStorage` write lands after the page has already read it; with no extension nobody answers, so it is a no-op.
- **manifest version** bumped to 0.25.11 so you can confirm the reloaded copy.
