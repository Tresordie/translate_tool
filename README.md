...
- **9 款主题一键切换** — 浅色（纸感白 Paper / 雾霭 Mist / 奶油 Cream / 晴空 Sky）× 深色（石墨 Graphite / 板岩 Slate / 午夜 Midnight / 轨道 Orbit / 夜曲 Nocturne）4+5 款，中性分层背景、克制强调色点睛、苹果式文字阶梯、极细中性边框 + 柔和弥散阴影，右下角悬浮面板切换，默认石墨黑（Graphite）；Orbit/Nocturne 为深空蓝黑×电光青/晴空蓝监控面板风格，源自《网页设计灵感参考库》配色体系
- **设计系统与可访问性基线** — 语义刻度令牌（6 级圆角 / 4px 基准间距 / 4 级高度阴影 / 语义光 / 三档动效时长）；`:focus-visible` 键盘焦点环（WCAG 2.4.7）；`prefers-reduced-motion` 全局动效降级；`@media print` 打印基线（报告/总结页可打印归档，暗色自动回落浅色面栈）；全站标题大纲 h1→h2→h3 三级台阶与等宽数字
- **品牌化标题排版** — 主标题采用 Plus Jakarta Sans 800 字标（异步加载），双色设计（"AI"渐变强调 + "Tool Box"实色）+ 工具箱渐变图标砖横向锁定，clamp 自适应全断点
- **打字机效果** — 翻译结果逐字显示，体验流畅
- **翻译历史** — 自动保存最近 20 条翻译记录，点击即可回填
- **便捷操作** — 一键交换语言、粘贴、清空、复制原文与翻译结果；粘贴一键完成，不再反复弹出剪贴板授权框
- **快捷键** — `Ctrl + Enter` 快速翻译
...
- **历史记录勾选导出** — 智能翻译、工作报告历史总结、英语学习、邮件总结、AI 解析、AI 提示词、微信工具（发送历史 + 总结记录）共 8 处历史区域支持逐条勾选 + 全选，把选中记录导出为 Markdown 或 HTML 文件（v0.45.0 起取代原先的"一键导出全部 JSON"）
- **微信格式一键转换** — 智能翻译（网页版 + 扩展全页）、工作报告、邮件总结、AI 解析、AI 提示词、微信工具（AI 总结）的结果区有「微信格式」按钮，一键把 AI 输出转成可直接粘贴到微信发送的纯文本，并尽量还原 Markdown 预览的结构：表格转为按列宽对齐、单元格自动折行的文本表格（含表头分隔线），标题分级符号（`【】`/`■`/`▍`），多行引用转 `▎` 引用块（`> [!warning]` 转图标），加粗的英文部分用 Unicode 粗体还原，列表转 `•`、任务项转 `☑/☐`，并清除微信会显示成方框的隐藏字符
...
- **邮件总结** — 新增第 5 个 Tab：粘贴邮件线程或上传本地文件（.txt/.md/.eml/.pdf），AI 按专业规范输出四段式详细总结（主题背景/时间线表格/技术要点/风险注意点）+ 按责任方分解的 To Do List（P0/P1/P2 优先级），支持 30 种语言输出、HTML/Markdown 下载、微信格式转换、历史记录查阅与编辑
...
- **数据与云同步（Google Drive）** — 所有数据本地存储（浏览器 localStorage + 服务 data/*.json），wechat_scheduler 服务作为同步枢纽把「全模块浏览器数据 + 微信任务/历史/AI 总结」打包写入 Google Drive 桌面客户端的本地同步文件夹（自动备份去抖 5s + 每日快照保留 14 份 + latest.json）。**每个模块页**（工作报告/任务清单/英语学习/邮件总结/AI解析/AI提示词/主页/微信工具）左下角「☁」胶囊点开即滑出云同步抽屉（`cloud-sync.js`）：配置 Drive 路径（内嵌目录浏览器 + 保存即试写）、自动备份开关、立即备份、从 Drive 恢复、快照逐个下载/恢复、导入/导出本地 JSON、开机自启。⚠️ 备份包**永不包含 API Key 与服务口令**，新设备恢复后重填一次 Key 即可
...
- **9 款主题切换** — 与网页版共享 theme.css 主题系统，克制配色 + 玻璃拟态质感
...
- **原文复制 / 微信格式** — 弹窗与全屏页原文区新增复制按钮；智能翻译（网页版 + 扩展全页）、工作报告、邮件总结、AI 解析、AI 提示词、微信工具（AI 总结）结果区均有「微信格式」一键转换，转出的纯文本可直接粘贴到微信发送
- **粘贴一键完成** — 原文粘贴按钮经扩展权限直接读取剪贴板，不再反复弹出「查看剪贴板」授权提示（网页版无扩展时自动降级为聚焦输入框 + `Ctrl+V` 提示，同样不弹授权框）
- **打开本地网页版** — Popup 头部新增按钮，一键在新标签页打开本地 `index.html`；页面路径可在设置面板「本地页面路径」中修改，未开启「允许访问文件网址」时给出开启指引
- **记录与配置全端互通** — 弹窗、侧边栏与网页版共享全部记录与配置：任一端产生的数据（含各模块历史）实时同步到其他端，历史支持勾选后导出为 Markdown / HTML 文件
...
- **右键菜单翻译** — 选中文字后右键选择「AI Tool Box 翻译」
- **9 款极简高级感主题与质感 UI** — 与网页版共享主题系统，克制低饱和配色、玻璃拟态卡片、细腻噪点背景与景深阴影
...
### v0.46.0 (2026-09-18)
- **智能翻译译文一键转微信格式（网页版 + 扩展全页）** — 智能翻译的译文区（网页 `index.html` 与扩展全页 `fullpage.html`）新增「微信格式」按钮：点击后在翻译卡片底部展开「💬 微信格式」区块，复用共享的 `markdownToWechat`（v0.45.0 结构还原版：表格文本对齐 / 标题分级符号 / 引用块 / Unicode 粗体）把译文转成可直接粘贴到微信发送的纯文本，区块内带独立「复制」按钮。**陈旧保护**：手动编辑译文、重新翻译、清空、恢复草稿时区块自动隐藏并清空，避免把过时转换结果粘进微信。至此微信格式转换覆盖 6 个模块：智能翻译、工作报告、邮件总结、AI 解析、AI 提示词、微信工具（AI 总结）。

### v0.45.0 (2026-09-17)
- **微信格式真正还原 Markdown 预览结构**（`markdown.js` 双副本，5 个模块共用）— 表格不再压成「值 | 值」管道串：按 CJK/emoji 各占 2 列计算显示宽度，超出 60 列预算时压缩最宽列并把单元格在列内折行，保持带表头分隔线的文本表格（`:---:` 对齐生效），仅列数 >6 才退回「▪ 首列 + 键：值」块；标题补齐全层级符号（h1 `【】` / h2 `■` / h3 `▍` / h4+ `▸`）；多行引用改为整块 `▎` 前缀且块内列表与序号保留，`> [!warning]` 类告警标注转成图标；加粗的 ASCII 部分用 Unicode 粗体还原（中文无对应字形，仍为普通字重）；代码块缩进不再被压平。
- **8 处历史区域统一「勾选 → 导出 MD / HTML」**— 智能翻译（网页版 + 扩展全页版）、工作报告历史总结、英语学习、邮件总结、AI 解析、AI 提示词、微信工具发送历史与总结记录。原先"一键导出全部为 JSON"的按钮按需求移除（翻译历史 / 工作记录 / 英语学习 / 邮件总结）；AI 解析的任务抽取历史转成任务表格 Markdown。
- **网页 ↔ 扩展历史同步的三处根因修复**— ① `manifest` 的 `content_scripts` 补 `all_frames: true`：index.html 内 7 个工具页都是 iframe，此前内容脚本只注入顶层帧，这些页面的记录既不上报也收不到扩展推送；② `AiService.onRecordSync` 网页分支只比较原始键，而 content.js 写的是映射后的带前缀键（`wr_work_records` / `td_todo_items`），改为同时匹配前缀键；③ index.html 顶层帧补 `record-sync` 订阅（同帧写 localStorage 不会触发本帧 storage 事件）。另修英语学习扩展页启动时无条件把本地旧副本推盖 `chrome.storage`（会覆盖网页端新记录），改为以 `chrome.storage` 为权威源、仅缺键时迁移。
- **输入内容保护原则：只有用户主动删除才清空**— 工作报告保存/更新记录后不再清空输入框，取消编辑恢复进入编辑前的内容；载入翻译历史 / 学习历史 / 编辑工作记录前弹确认，取消则完全不覆盖；任务清单 Markdown 导入取消与导入成功后保留已粘贴内容；微信工具保存任务后保留表单内容（「重置」按钮仍为主动清空）。
- **智能翻译输入区可拖拽调整高度**（默认 300px，`resize: vertical`；同时取消 flex 拉伸，否则拖拽会被拉回）。**修复切换 Markdown 预览导致中文输入法未上屏内容丢失**（预览按钮 `mousedown` 阻止默认，不再抢走输入框焦点），并补上扩展全页版与弹窗版**从未绑定的预览按钮**（MV3 迁移删掉内联 `onclick` 后未补监听）。
- **微信工具「AI 总结」按钮永久发灰修复**— 标签写死 `dim-50` 且读取失败后不恢复禁用态；改为默认可用 + 点击时校验（未读取聊天记录给出提示）+ `finally` 统一恢复。
- 顺带修复：翻译页两栏输入框与分区线错位（`theme.css` 的 `:where(textarea.md-editor){width:100%}` 使左栏溢出压线，页面内 `width:auto` 局部覆盖）；工作报告 `init()` 漏读 `work_summaries` 导致刷新后「历史总结」恒为空。

### v0.44.0 (2026-09-16)
- **同步覆盖审计 + 网页↔扩展记录互通补全** — 系统性扫描 8 模块全部存储键 vs 双向映射表 vs Drive 收集范围，补齐缺口：邮件总结草稿（`email_summary_draft`）自动保存纳入反向 relay；微信工具 5 键（服务地址/令牌/风险确认/总结语言/联系人缓存）进入 record 双向映射（`ws_api_token` 仅本地双向一致，Drive 推送仍被 SECRET_DROP 剔除）；微信工具页补网页分支 relay 与双环境实时缓存刷新。Drive 同步链路（全量收集 + 剔密）确认覆盖全部模块记录。

### v0.43.0 (2026-09-16)
- **修复英语学习页扩展环境完全失效的真正根因：MV3 CSP 阻止内联脚本** — 扩展版 `english_learning.html` 是全站唯一将全部逻辑写在内联 `<script>` 的模块页，MV3 默认 CSP `script-src 'self'` 直接阻止执行 → 配置加载/同步/全部交互在扩展环境中静默死掉（页面渲染正常极具迷惑性）。修复：998 行内联脚本提取为外部 `english_learning.js`（双副本），字体行 `onload` 内联属性同步改 `id="gfAsync"`（CSP 同样阻止内联事件处理器）。

### v0.42.0 (2026-09-16)
- **修复 Chrome 114+ onChanged 签名变更导致的全站实时同步失效** — Chrome 114 起回调第二参数由 string `'local'` 改为 context 对象，16 文件 19 处 `area === 'local'` 判断在新版 Chrome 永远为 false，所有实时监听（background 广播/content.js 中继/各页监听）静默失效。统一改为双签名兼容，旧 string/新 context 双模式单元验证通过。

### v0.41.0 (2026-09-16)
- **英语学习页接入统一配置同步通道** — 复用其他页面已验证可用的 `AiService.initConfigSync`（扩展环境 chrome.storage + 网页环境 postMessage），与既有 storage/专用监听形成多路冗余；任务清单品牌区对齐子页 hero 规格。

### v0.40.0 (2026-09-16)
- **主题体系收敛至 9 款** — 移除书斋 Study / 鎏金 Navy / 极光 Aurora 三套灵感库主题；补全 `--green/--green-a12/--cyan-a12` 缺失变量（修复"保存配置/保存今天内容"按钮透明回归）；page-hero 改横向锁定布局；字标统一 Plus Jakarta Sans 800 修复宽扁观感；英语学习页补 storage 事件通道与启动竞态守卫。

### v0.39.0 (2026-09-16)
- **全站图文标题（page-hero）体系** — theme.css 新增 `.page-hero` 组件：渐变图标砖 + 双色字标 + 功能副题 + 渐变发丝线，index/全页头升级；各子页 glyph 与功能匹配（信封/闪电/双星/文档柱图/聊天气泡）；英语学习卡内标题实色化。

### v0.38.0 (2026-09-16)
- **《页面设计评审报告》十项全量落实** — theme.css 全局层（语义刻度令牌 6 级圆角/8 级间距/4 级高度阴影/语义光/三档动效/字号刻度）；全局 `:focus-visible`（WCAG 2.4.7）；`prefers-reduced-motion` 全局动效降级；`@media print` 打印基线；6 页标题大纲补 h2 台阶；字体统一去 Sora；english_learning 去 Tailwind 硬编码色；wechat 32 处内联样式提取；断点补齐（平板档 + 触屏降级）；新建 `ai-panel.css` 抽取三页 57 条公共规则。

### v0.37.0 (2026-09-16)
- **灵感库设计体系落地（全站分批改版第 1 批）** — 新增 5 套主题（夜曲/鎏金/极光/书斋/晴空）+ 任务清单页 Bento KPI 指标卡（等宽大数字 + 渐变进度条 + 大写眉标）。

### v0.36.0 (2026-09-16)
- **新增第 7 款主题「轨道 · Orbit」** — 深空蓝黑×电光青监控面板风格，扁平卡片 + 极细描边。

### v0.35.0 (2026-09-16)
- **任务清单看板质感精修** — 渐变标题/图标砖、列顶状态色发丝线、卡片优先级左色条、玻璃分层阴影、拖拽倾斜、入场瀑布动画。

### v0.34.0 (2026-09-16)
- **任务清单看板化重设计**（Roadmaps Board 风格）— 四列看板（待办/进行中/已完成/已暂停）+ 拖拽换列 + 数据模型新增 `status/desc/icon` 字段（旧数据自动迁移）+ 顶栏统计与筛选 + 三弹层交互；存储键不变，Google Drive 云同步零改动。
...
### v0.31.0 (2026-09-14)
- **全模块云同步控制** — 新增自包含组件 `cloud-sync.js`：工作报告/任务清单/英语学习/邮件总结/AI解析/AI提示词/主页 与微信工具页一样，点各页左下角「☁」胶囊即滑出**云同步抽屉**（Drive 路径 + 内嵌目录选择弹窗、自动备份开关、立即备份、从 Drive 恢复、快照逐个下载/恢复、导入/导出本地 JSON、开机自启），无需跳转即可在任意模块页管理同步。数据本就在备份包内，至此每页都有完整控制入口。
- **验证** — 全模块编译/语法通过；e2e 46 项、逻辑 8 项、清洗 7 项全绿。
...
- **新增：智能翻译原文输入框复制按钮** — 网页版与扩展（弹窗 / 全屏页 / 侧边栏）左侧原文工具条新增复制按钮，一键复制原文（空输入时提示且不覆盖剪贴板）。
- **修复：工作报告「总结输出语言选 English 却输出中文」** — 根因是配置同步：`applySyncedConfig` 每次收到插件广播都把 `config.outputLang` 重置为 `zh` 且从不回写下拉框，界面显示 English 而生成时读到的却是 `zh`，于是走了中文 prompt 分支。现生成时以下拉框当前值为准，配置同步只更新 Base URL / API Key / 模型并保留语言偏好、回写下拉框。
- **修复：粘贴按钮每次都弹剪贴板授权框** — `navigator.clipboard.readText()` 受权限门控：`file://` 页面属不透明来源、浏览器不保留其授权，故每次调用都会弹框；扩展页同样弹框（manifest 未声明 `clipboardRead`）。现扩展声明 `clipboardRead` 权限，网页版经内容脚本 `read-clipboard` 桥用 `execCommand('paste')` 代读剪贴板（一键粘贴且不弹框）；无扩展或权限为 `prompt` 时改为聚焦输入框并提示按 `Ctrl+V`，**任何情况都不再触发授权对话框**。扩展全屏页 / 侧边栏改用 `execCommand('paste')` 优先、`readText` 兜底。⚠️ **需在 `chrome://extensions` 重新加载扩展**（新增权限）。
- **新增：Popup「打开本地网页版」入口** — Popup 头部新增按钮，一键在新标签页打开本地 `index.html`；路径可在设置面板「本地页面路径」中修改（默认指向仓库内 `index.html` 的本地路径），未开启「允许访问文件网址」时给出开启指引并提示失败原因。
- **修复：首开配置竞态** — 页面初始化时主动向扩展请求一次配置（`request-config` 握手），绕开 content script 异步写 `localStorage` 可能晚于页面初始化读取的竞态；无扩展环境无人应答，零副作用。
- **manifest 版本号** 同步至 0.25.11（便于确认加载的是新副本）。
