...
- **微信格式一键转换** — 工作报告、邮件总结、AI 解析、AI 提示词四个模块的结果区有「微信格式」按钮，一键把 AI 输出转成可直接粘贴到微信发送的纯文本，并尽量还原 Markdown 预览的结构：表格转为按列宽对齐、单元格自动折行的文本表格（含表头分隔线），标题分级符号（`【】`/`■`/`▍`），多行引用转 `▎` 引用块（`> [!warning]` 转图标），加粗的英文部分用 Unicode 粗体还原，列表转 `•`、任务项转 `☑/☐`，并清除微信会显示成方框的隐藏字符
...
- **原文复制 / 微信格式** — 弹窗与全屏页原文区新增复制按钮；工作报告、邮件总结、AI 解析、AI 提示词结果区新增「微信格式」一键转换，转出的纯文本可直接粘贴到微信发送
