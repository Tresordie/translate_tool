...
- **热点雷达** — 新增第 8 个 Tab：用户创建多张热点卡片（每张卡片一个提示词，用于归类），AI 实时抓取微博/知乎/百度/抖音/B站/IT之家/36氪 等全网热榜并按提示词筛选出最相关的 Top 10；卡片含排名色阶、来源标签、可点击原文链接、热度值与刷新按钮（点击后 AI 重新检索更新）
...
- **Side Panel 侧边栏** — Chrome 114+ 专属侧边栏，内置智能翻译/工作报告/任务清单/英语学习/邮件总结/AI 解析/AI 提示词全部 7 个模块，Tab 一键切换，模块懒加载不卡顿；可通过 Popup 侧边栏按钮、快捷键 `Alt+Shift+L` 或右键菜单「在侧边栏打开 AI Tool Box」随时打开
