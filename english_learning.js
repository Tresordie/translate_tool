...
// ===== Chrome Extension Storage Adapter =====
        ...
        function loadApiConfig() {
            if (window.__elExtSynced) return; // v0.40.0: 已被扩展同步覆盖时，不用旧 localStorage 值回填
            var config;
        ...
        // v0.40.0 配置同步补强：storage 事件通道 —— 顶层 content.js 写 translate_config、
        // 桥接适配器写 config；postMessage 只触达顶层窗口，跨窗口/iframe 场景 storage 事件才是可靠通道
        window.addEventListener('storage', function (e) {
            if (!e.newValue || (e.key !== 'translate_config' && e.key !== 'config')) return;
            try {
                var c = JSON.parse(e.newValue);
                if (c && c.baseUrl) applySyncedApiConfig(c);
            } catch (err) {}
        });
        // v0.41.0 复用其他页面已验证可用的统一配置同步通道（AiService.initConfigSync：
        // 扩展环境 chrome.storage.onChanged + 启动 get('config')；网页环境 postMessage）
        (function () {
            try {
                var Ai = window.AiService;
                if (Ai && typeof Ai.initConfigSync === 'function') {
                    Ai.initConfigSync(function (cfg) {
                        if (cfg && cfg.baseUrl) applySyncedApiConfig(cfg);
                    });
                }
            } catch (e) {}
        });
        ...
        function showSuccess(message) {
            ...
            errorEl.style.background = 'var(--green-a08)';
            errorEl.style.color = 'var(--green)';
            errorEl.style.border = '1px solid var(--green-a15)';
